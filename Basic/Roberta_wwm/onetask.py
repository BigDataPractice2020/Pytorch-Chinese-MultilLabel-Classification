...
#CUDA_DEVICE_ORDER="PCI_BUS_ID"  CUDA_VISIBLE_DEVICES=2 python onetask.py
...
label = []         # 标签
pad_size = 36   # 也称为 max_len (前期统计分析，文本长度最大值为38，取32即可覆盖99%)
...
class Model(nn.Module):
    def __init__(self):
        super(Model, self).__init__()
        bert = BertModel.from_pretrained(bert_path)  # /bert_pretrain/
        net = nn.ModuleList([bert])
        layers_to_drop = 2
        self.bert = Layerdrop(net, layers_to_drop)
        for param in self.bert.parameters():
            param.requires_grad = True  # 每个参数都要 求梯度 feiyu 改成false
        self.fc = nn.Linear(768, 21)   # 768 -> 2
