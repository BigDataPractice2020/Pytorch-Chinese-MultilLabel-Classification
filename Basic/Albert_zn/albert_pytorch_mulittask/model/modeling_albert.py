...
class AlbertForSequenceClassification(AlbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # 根据config定义一个和预训练模型一致的albert模型
        self.bert = AlbertModel(config)
        self.dropout = nn.Dropout(0.1 if config.hidden_dropout_prob == 0 else config.hidden_dropout_prob)
        self.classifier_list = nn.ModuleList()
        # 问答系统可以分为7个环节
        for i in range(7):
            self.classifier_list.append(nn.Linear(config.hidden_size, self.config.num_labels))
        self.init_weights()

    def forward(self, input_ids, num_link,attention_mask=None, token_type_ids=None,
        ...
        outputs = self.bert(input_ids,
                            ...
                            head_mask=head_mask)
        # output[0]->(48,60,1024)
        # output[1]->(48,1024)->（batch,feature）
        pooled_output = outputs[1]
        pooled_output = self.dropout(pooled_output+0.1)
        # logits未进入softmax的
        # 不同的环节进入不同的全连接
        logits = self.classifier_list[num_link](pooled_output)
        #实际上这里idx2之后没东西 啥都没加
