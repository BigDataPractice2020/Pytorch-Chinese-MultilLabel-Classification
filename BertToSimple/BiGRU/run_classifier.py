...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # print(inputs)
            loss_and_logits = model(**inputs)
            loss = loss_and_logits[0]  # model outputs are always tuple in transformers (see doc)
            if args.n_gpu > 1:
        ...
        if 'cuda' in str(args.device):
            del batch,inputs,loss_and_logits
            torch.cuda.empty_cache()
    return global_step, tr_loss / global_step

def evaluate(args, model, tokenizer, prefix="",log_confusion=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        #　这里如果用train是需要得到训练集数据的logits，为之后蒸馏做准备。
        ...
        with torch.no_grad():
            for step, batch in enumerate(eval_dataloader):
                ...
                if preds is None:
                    ...
                    out_label_ids = np.append(out_label_ids, inputs['labels'].detach().cpu().numpy()[:,0])
                pbar(step)
        ...
        if args.output_mode == "classification" and eval_task == "mrpc":
            if log_confusion == True:
                label_pre_list = []
                confusion_list = []
                # 7个环节 每个环节是3类
                for i in range(7):
                    confusion_list.append(np.zeros((3,3)))

            start_idx_list = list(range(0,21,3))
            final_preds = []
            assert len(out_label_ids) == len(preds)


            for i in range(len(preds)):
                now_pred_data = preds[i]
                now_label = out_label_ids[i]
                for link_num,start_idx in enumerate(start_idx_list,0):
                    if now_label>=start_idx and now_label<(start_idx+3):
                        break
                # 根据label找出对应区间段的预测概率
                now_pred_data = now_pred_data[start_idx:start_idx+3]
                now_pred_idx = np.argmax(now_pred_data)
                now_pred_idx = start_idx+now_pred_idx
                final_preds.append(now_pred_idx)
                # 记录混淆矩阵
                if log_confusion == True:
                    label_pre_list.append([now_label, now_pred_idx])
                    link_pre = now_pred_idx-start_idx
                    link_label = now_label-start_idx
                    confusion_list[link_num][link_label][link_pre] += 1
            # 保存混淆矩阵
            if log_confusion == True:
                save_csv = pd.DataFrame(label_pre_list)
                save_csv.columns = ["label","pre"]
                save_csv.index = list(range(len(save_csv)))
                save_csv.to_csv("./label_and_pre.csv",header=True,index=True)
                for i in range(7):
                    np.save("./confusion/confusion{}.npy".format(i),confusion_list[i])
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    if os.path.exists(cached_features_file):
        ...
        # mnli：语型内匹配，判断两个句子意思相近？ mnli-mm：跨语型匹配，两个句子意思相近？
        #　roberta一种bert的改进版本
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default="./prev_trained_model/albert_21_93", type=str, required=False,
    ...
    # 从头训练蒸馏模型还是加载蒸馏模型
    parser.add_argument("--continue_train_distill", default=True, type=bool, required=False,
                        help="是否加载蒸馏一半的模型")
    # 蒸馏模型的位置
    parser.add_argument("--distill_model_dir", default="./good_result/pytorch_model-90.bin",
                        type=str, required=False,help="是否加载蒸馏一半的模型")
    # 继续蒸馏的学习率
    parser.add_argument("--continue_learning_rate", default=5e-5, type=float,
    ...
    # 每多少个batch评估一次eval 700
    parser.add_argument('--logging_steps', type=int, default=300,
                        help="Log every X updates steps.")
    # 每多少batch保存一次模型
    parser.add_argument('--save_steps', type=int, default=900,
    ...
    # Setup CUDA, GPU & distributed training/设置GPU或者分布式训练
    ...
    model = my_model.DistillModel(args, config)
    if args.continue_train_distill == True:
        model.load_state_dict(torch.load(args.distill_model_dir))
        print("*"*50)
        print("从{}加载训练一半的模型继续训练，学习率为{}".format(args.distill_model_dir,args.continue_learning_rate))
        print("*" * 50)
        args.learning_rate = args.continue_learning_rate

    else:
        print("*" * 50)
        print("从头训练，学习率为{}".format(args.learning_rate))
        print("*" * 50)
